...
Since there are no numerical data to be checked for outliers, and all attribute types are char, we can't calculate variance or do discretization.
...
As we can see, the dataset looks so much better now and is nearly perfectly balanced
...
The 70/30 split with Gain Ratio showed good results across the evaluation metrics, with accuracy at 90.84%, precision which is the positive predictive value 92.56%, sensitivity which is the true positive rate at 91.47%, and specificity which is the true negative rate at 95.47%, those are very good numbers and indicate a very good model.
...
#### Cluster Validation Using the Silhouette Method:
...
K=2 seems to be the optimal number of clusters using the silhouette method.
...
# Visualize the clustering
fviz_cluster(k3_results, data = df_numeric, geom = "point")

# Calculate the silhouette width
silhouette_score <- silhouette(k3_results$cluster, dist(df_numeric))
# Extract the average silhouette width
average_silhouette <- mean(silhouette_score[, 3])

within_cluster_sum_squares <- k3_results$tot.withinss

# BCubed function
clusters <- k3_results$cluster
ground_truth <- c(balancedDataset$Adaptivity.Level)
...
|                                    |    K=2    |    K=3    |    K=4    |
|:----------------------------------:|:---------:|:---------:|:---------:|
|       Avg. Silhouette Width        | 0.3145231 | 0.2869528 | 0.253453  |
| Total Within-Cluster Sum of Square | 5062.039  | 4446.174  |  3933.21  |
...
From the results, we found that our dataset is unsuitable for clustering since the clusters overlap in all the plots we made, but despite that 2 clusters did show better results as the silhouette and elbow methods suggested. We believe this is due to the inherent nature of categorical data. An article on Safe Hub Collective stated that "k-Means algorithm is not applicable to categorical data, as categorical variables are discrete and do not have any natural origin. So computing euclidean distance for such as space is not meaningful."[1]
